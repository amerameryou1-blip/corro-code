---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Corro Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Corro Code bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Corro Code-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/corro/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Corro Code-konfigurationen.
...
Corro Code Zen er en liste over modeller leveret af Corro Code-teamet, som er blevet testet og verificeret til at fungere godt med Corro Code. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Corro Code Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg corro og gå til [corrocode.dev/auth](https://corrocode.dev/auth).
...
Det fungerer som alle andre udbydere i Corro Code og er helt valgfrit at bruge.

---

## Corro Code Go

Corro Code Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Corro Code-teamet, som er testet og verificeret til at fungere godt med Corro Code.

1. Kør kommandoen `/connect` i TUI, vælg `Corro Code Go`, og gå til [corrocode.dev/auth](https://corrocode.dev/zen).
...
Sådan bruger du Amazon Bedrock med Corro Code:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører corro:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `corro.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Corro Code understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere corro til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at corro skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem corro-konfigurationen.
...
gemme tokenet i corro auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Corro Code bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Corro Code
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`corro.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [corro-gitlab-auth](https://www.npmjs.com/package/corro-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `corro.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Corro Code:
...
Sådan bruger du Google Vertex AI med Corro Code:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem corro, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [corro-helicone-session](https://github.com/H2Shami/corro-helicone-session) plugin for automatisk at logge hver Corro Code-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Corro Code-samtale opført som en separat session.
...
Du kan konfigurere corro til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere corro til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere corro til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Corro Code. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/corro) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Corro Code:
...
3. Kopier API-nøglen til brug i Corro Code.
...
6. **Vigtigt**: Før du bruger skymodeller i Corro Code, skal du hente modelinformationen lokalt:
...
Corro Code Zen er en liste over testede og verificerede modeller leveret af Corro Code-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Corro Code Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Corro Code Zen**.

   ```txt
   /connect
   ```

3. Indtast Corro Code API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem corro-konfigurationen.
...
5. Du kan også tilpasse dem gennem corro-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Corro Code:
...
Du kan også tilpasse modeller gennem corro-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med corro. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `corro.json`-filen i projektkataloget:
...
`limit`-felterne lader Corro Code forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `corro auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek corro-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i corro-konfigurationen.
