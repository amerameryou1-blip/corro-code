...
Delete `packages/corro/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/corro/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/corro/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/corro`
- TUI plugin loader tests in `packages/corro` if runtime plugin API wiring changed
