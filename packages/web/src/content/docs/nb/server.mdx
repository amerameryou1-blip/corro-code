---
title: Tjener
description: Samhandle med Corro Code-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `corro serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Corro Code-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `corro.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `corro` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `corro serve` og `corro web`.
...
Når du kjører `corro` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Corro Code-serveren til å samhandle med Corro Code programmatisk.
:::

Denne arkitekturen lar Corro Code støtte flere klienter og lar deg samhandle med Corro Code programmatisk.

Du kan kjøre `corro serve` for å starte en frittstående server. Hvis du har
Corro Code TUI kjører, vil `corro serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Corro Code [IDE](/docs/ide) plugins.
...
corro-serveren viser følgende APIer.
