...
For at fejlfinde problemer med Corro Code, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\corro\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `corro --log-level DEBUG`.

---

## Lagring

corro gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/corro/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\corro`
...
Corro Code Desktop kører en lokal Corro Code-server (`corro-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Corro Code`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/corro/corro.jsonc` (eller `~/.config/corro/corro.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/corro/corro.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\corro\corro.jsonc`
...
Corro Code kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/corro/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\corro\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Corro Code kan genopbygge den.

1. Afslut Corro Code Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/corro`
- **Linux**: slet `~/.cache/corro` (eller kør `rm -rf ~/.cache/corro`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\corro`

3. Start Corro Code Desktop på ny.

---

### Løs problemer med serverforbindelse

Corro Code Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `corro.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Corro Code Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Corro Code.

---

### Meddelelser vises ikke

Corro Code Desktop viser kun systemvarsler når:

- varsler er aktiveret for Corro Code i dine OS-indstillinger, og
...
1. Afslut Corro Code Desktop.
2. Find og slet disse filer (de findes i Corro Code Desktop-appens datamappe):

- `corro.settings.dat` (skrivebordsstandardserver URL)
- `corro.global.dat` og `corro.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Corro Code:
...
For at finde ud af hvilke modeller du har adgang til, kør `corro models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\corro`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. corro installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\corro`

2. Start corro på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge corro til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
corro vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
