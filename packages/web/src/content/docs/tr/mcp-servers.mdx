...
_Model Bağlam Protokolü_ veya MCP kullanarak corro'a harici araçlar ekleyebilirsiniz. corro hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [corro Config](https://corrocode.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/corro` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
corro, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde corro şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa corro, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `corro mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından corro, belirteçleri `~/.local/share/corro/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra corro'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi corro yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve corro'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
