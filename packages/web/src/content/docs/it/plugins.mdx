---
title: Plugin
description: Scrivi plugin per estendere Corro Code.
---

I plugin ti permettono di estendere Corro Code agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Corro Code.
...
- `.corro/plugins/` - plugin a livello progetto
- `~/.config/corro/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/corro/node_modules/`.
...
1. Config globale (`~/.config/corro/corro.json`)
2. Config di progetto (`corro.json`)
3. Directory plugin globale (`~/.config/corro/plugins/`)
4. Directory plugin di progetto (`.corro/plugins/`)
...
Corro Code esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di corro per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere corro.
...
:::note
Se usi l'app desktop di Corro Code, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a corro di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a corro:
...
L'helper `tool` crea uno strumento personalizzato che corro può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in corro insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://corrocode.dev/docs/sdk) per i dettagli.
