...
Postoje dvije vrste agenata u Corro Code: primarni agenti i podagenti.
...
Corro Code dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Corro Code dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Corro Code dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Corro Code-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `corro.json`:
...
- Globalno: `~/.config/corro/agents/`
- Po projektu: `.corro/agents/`
...
Ako temperatura nije navedena, Corro Code koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Corro Code konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Corro Code konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Corro Code Zen](/docs/zen), koristili biste `corro/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `corro models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/corro).
:::
