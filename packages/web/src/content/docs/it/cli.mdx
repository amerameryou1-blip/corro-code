---
title: CLI
description: Opzioni e comandi della CLI di Corro Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Corro Code, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
corro
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Corro Code in modo programmatico.
...
Avvia l'interfaccia testuale di Corro Code (interfaccia utente da terminale).
...
La CLI di Corro Code include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Corro Code.
...
Collega un terminale a un backend server di Corro Code già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
corro attach [url]
```

Questo consente di usare la TUI con un backend Corro Code remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `corro`) |
...
Corro Code si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `corro auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

Quando Corro Code si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Corro Code.
...
Esegui corro in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `corro serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server corro in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `corro`) |
...
Avvia un server Corro Code headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
corro serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di corro senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `corro`).
...
Gestisci le sessioni Corro Code.
...
Elenca tutte le sessioni Corro Code.
...
Mostra statistiche di utilizzo token e costo per le sessioni Corro Code.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Corro Code.

```bash
corro import <file>
```

Puoi importare da un file locale o da un URL di condivisione Corro Code.
...
Avvia un server Corro Code headless con interfaccia web.

```bash
corro web
```

Avvia un server HTTP e apre un browser per accedere a Corro Code tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `corro`).
...
Disinstalla Corro Code e rimuove tutti i file correlati.
...
Aggiorna corro all'ultima versione o a una versione specifica.
...
La CLI di corro accetta i seguenti flag globali.
...
Corro Code può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `corro`)     |
