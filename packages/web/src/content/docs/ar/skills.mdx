...
تتيح مهارات الوكيل لـ Corro Code اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Corro Code في هذه المواقع:

- إعدادات المشروع: `.corro/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/corro/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Corro Code صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.corro/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/corro/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.corro/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Corro Code المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `corro.json`:
...
**للوكلاء المدمجين** (في `corro.json`):
