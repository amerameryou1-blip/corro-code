...
As habilidades do agente permitem que o corro descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O corro pesquisa nesses locais:

- Configuração do projeto: `.corro/skills/<name>/SKILL.md`
- Configuração global: `~/.config/corro/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o corro sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.corro/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/corro/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.corro/skills/git-release/SKILL.md` assim:
...
O corro lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `corro.json`:
...
**Para agentes embutidos** (em `corro.json`):
