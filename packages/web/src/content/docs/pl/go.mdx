...
Corro Code Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Corro Code. Subskrybujesz Corro Code Go i
...
Corro Code Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

Corro Code Go działa jak każdy inny dostawca w Corro Code.

1. Logujesz się do **<a href={console}>Corro Code Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Corro Code Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Corro Code Go.
:::
...
Corro Code Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Corro Code
używa formatu `corro-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`corro-go/kimi-k3` w swojej konfiguracji.
...
Stworzyliśmy Corro Code Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Corro Code.
