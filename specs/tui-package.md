...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/corro`.
- Do not replace private `packages/corro` imports with `@corro-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@corro-ai/tui`.
...
Status: Completed. The private `@corro-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@corro-ai/tui`.
...
- The package has no dependency on `corro`, `@corro-ai/cli`, or
  `@corro-ai/core`.
...
primitives now live in `@corro-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@corro-ai/tui/runtime`. Movable app, component,
...
`@corro-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@corro-ai/tui`. Configured references resolve through the new
...
`@corro-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@corro-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@corro-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@corro-ai/core`, or either
  executable package.
...
rg '@corro-ai/core|packages/corro|packages/cli' packages/tui
