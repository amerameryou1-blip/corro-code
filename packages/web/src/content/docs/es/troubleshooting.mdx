...
Para depurar problemas con Corro Code, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/corro/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\corro\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `corro --log-level DEBUG`.

---

## Almacenamiento

corro almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/corro/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\corro`
...
Corro Code Desktop ejecuta un servidor Corro Code local (el sidecar `corro-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Corro Code` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/corro/corro.jsonc` (o `~/.config/corro/corro.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/corro/corro.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\corro\corro.jsonc`
...
Corro Code también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/corro/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\corro\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.corro/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Corro Code pueda reconstruirlo.

1. Salga de Corro Code Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/corro`
- **Linux**: eliminar `~/.cache/corro` (o ejecutar `rm -rf ~/.cache/corro`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\corro`

3. Reinicie el escritorio Corro Code.

---

### Solucionar problemas de conexión del servidor

Corro Code Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `corro.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio Corro Code requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Corro Code.

---

### Notificaciones que no se muestran

Corro Code Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Corro Code en la configuración de su sistema operativo, y
...
1. Salga del escritorio Corro Code.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Corro Code):

- `corro.settings.dat` (URL del servidor predeterminado de escritorio)
- `corro.global.dat` y `corro.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Corro Code:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/corro/issues**](https://github.com/anomalyco/corro/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**corrocode.dev/discord**](https://corrocode.dev/discord)
...
3. Asegúrese de tener la última versión con `corro upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `corro models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/corro
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\corro`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. corro instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\corro`

2. Reinicie corro para reinstalar los paquetes del proveedor más recientes.

Esto obligará a corro a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
corro detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
