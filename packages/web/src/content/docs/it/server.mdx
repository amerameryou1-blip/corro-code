---
title: Server
description: Interagisci con il server corro via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `corro serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client corro.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `corro.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `corro`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `corro serve` sia per `corro web`.
...
Quando esegui `corro` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server corro per interagire con corro in modo programmatico.
:::

Questa architettura permette a corro di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `corro serve` per avviare un server standalone. Se la TUI di corro e gia in esecuzione, `corro serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Corro Code.
...
Il server corro espone le seguenti API.
