...
Pour déboguer les problèmes avec Corro Code, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\corro\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `corro --log-level DEBUG`.

---

## Stockage

corro stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/corro/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\corro`
...
Corro Code Desktop exécute un serveur Corro Code local (le side-car `corro-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Corro Code` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/corro/corro.jsonc` (ou `~/.config/corro/corro.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/corro/corro.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\corro\corro.jsonc`
...
Corro Code peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/corro/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\corro\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Corro Code puisse le reconstruire.

1. Quittez complètement Corro Code Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/corro`
- **Linux** : supprimez `~/.cache/corro` (ou exécutez `rm -rf ~/.cache/corro`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\corro`

3. Redémarrez le bureau Corro Code.

---

### Résoudre les problèmes de connexion au serveur

Corro Code Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `corro.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, Corro Code Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Corro Code.

---

### Les notifications ne s'affichent pas

Corro Code Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Corro Code dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Corro Code.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Corro Code Desktop) :

- `corro.settings.dat` (serveur de bureau par défaut URL)
- `corro.global.dat` et `corro.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Corro Code :
...
3. Assurez-vous d'avoir la dernière version avec `corro upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `corro models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\corro`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. corro installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\corro`

2. Redémarrez corro pour réinstaller les derniers packages du fournisseur

Cela forcera corro à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
corro détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
