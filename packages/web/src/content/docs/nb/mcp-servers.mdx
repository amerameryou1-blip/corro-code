...
Du kan legge til eksterne verktøy i Corro Code ved å bruke _Model Context Protocol_ eller MCP. Corro Code støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Corro Code Config](https://corrocode.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="corro.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/corro`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Corro Code håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Corro Code:
...
Hvis serveren krever autentisering, vil Corro Code be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `corro mcp auth <server-name>`.
...
```bash
corro mcp auth my-oauth-server
```
...
```bash
corro mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Corro Code lagre tokens sikkert i `~/.local/share/corro/mcp-auth.json`.
...
corro mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Corro Code, sammen med innebygde verktøy. Så du kan administrere dem gjennom Corro Code-konfigurasjonen som alle andre verktøy.
...
```json title="corro.json" {11, 14-18}
{
  "$schema": "https://corrocode.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
corro mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Corro Code til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Corro Code-samtale oppført som en separat økt.
