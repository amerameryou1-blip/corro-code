...
description: Skriv dine egne programtillegg for å utvide Corro Code.
---

Plugins lar deg utvide Corro Code ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Corro Code.
...
- `.corro/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/corro/node_modules/`.
...
1. Global konfigurasjon (`~/.config/corro/corro.json`)
2. Prosjektkonfigurasjon (`corro.json`)
3. Global plugin-katalog (`~/.config/corro/plugins/`)
4. Prosjektpluginkatalog (`.corro/plugins/`)
...
Corro Code kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Corro Code SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Corro Code.
...
:::note
Hvis du bruker Corro Code-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Corro Code fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Corro Code:
...
`tool`-hjelperen lager et tilpasset verktøy som Corro Code kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Corro Code sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://corrocode.dev/docs/sdk) for detaljer.
