---
title: 플러그인
description: Corro Code를 확장하기 위해 자신만의 플러그인을 작성하세요.
---
...
- `.corro/plugins/` - 프로젝트 레벨 플러그인
- `~/.config/corro/plugins/` - 글로벌 플러그인
...
**npm 플러그인**은 시작시 Bun을 사용하여 자동으로 설치됩니다. 패키지와 그 의존성은 `~/.cache/corro/node_modules/`에 캐시됩니다.
...
1. 글로벌 구성 (`~/.config/corro/corro.json`)
2. 프로젝트 구성 (`corro.json`)
3. 글로벌 플러그인 디렉토리 (`~/.config/corro/plugins/`)
4. 프로젝트 플러그인 디렉토리 (`.corro/plugins/`)
...
- `client`: AI와 상호 작용을 위한 corro SDK 클라이언트.
...
:::note
corro 데스크톱 앱을 사용하는 경우 응답이 준비되어 있거나 세션 오류가 있을 때 시스템 알림을 자동으로 보낼 수 있습니다.
:::
...
레벨: `debug`, `info`, `warn`, `error`. [SDK 문서](https://corrocode.dev/docs/sdk)를 참고하세요.
