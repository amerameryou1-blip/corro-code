---
title: CLI
description: Opciones y comandos de la CLI de Corro Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Corro Code de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
corro
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Corro Code mediante programación.
...
Inicie la interfaz de usuario del terminal Corro Code.
...
La CLI de Corro Code también tiene los siguientes comandos.

---

### agent

Administrar agentes para Corro Code.
...
Conecte una terminal a un servidor backend Corro Code que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
corro attach [url]
```

Esto permite usar TUI con un backend remoto Corro Code. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `corro`) |
...
Corro Code funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `corro auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

Cuando se inicia Corro Code, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Corro Code.
...
Ejecute corro en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `corro serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor corro en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `corro`) |
...
Inicie un servidor Corro Code sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
corro serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad corro sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `corro`).
...
Administrar Corro Code sesiones.
...
Enumere todas las sesiones Corro Code.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Corro Code.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Corro Code.

```bash
corro import <file>
```

Puede importar desde un archivo local o una URL compartida Corro Code.
...
Inicie un servidor Corro Code sin cabeza con una interfaz web.

```bash
corro web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Corro Code a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `corro`).
...
Desinstale Corro Code y elimine todos los archivos relacionados.
...
Actualiza corro a la última versión o a una versión específica.
...
La CLI de Corro Code toma las siguientes banderas globales.
...
Corro Code se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `corro`) |
