...
describe("public import boundaries", () => {
  test("isolates each public entrypoint", async () => {
    ...
    const promiseRpc = await bundleInputs("@opencode-ai/client/promise/websocket", "browser")
    expect(within(promiseRpc.eager, effect).length).toBeGreaterThan(0)
    expect(within(promiseRpc.eager, protocol).length).toBeGreaterThan(0)
    expect(within(promiseRpc.all, core)).toEqual([])
    expect(within(promiseRpc.all, server)).toEqual([])
