...
corro ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak corro kendi `corro` temasını kullanır.
...
corro birden fazla yerleşik temayla gelir.
...
- corro'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
corro, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/corro/themes/*.json` veya `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.corro/themes/*.json`
4. **Geçerli çalışma dizini** - `./.corro/themes/*.json`
