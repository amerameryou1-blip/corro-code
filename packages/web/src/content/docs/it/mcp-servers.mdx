...
Puoi aggiungere strumenti esterni a Corro Code usando il _Model Context Protocol_, o MCP. Corro Code supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Corro Code Config](https://corrocode.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/corro`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Corro Code gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Corro Code:
...
Se il server richiede autenticazione, Corro Code ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `corro mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Corro Code memorizzera' i token in modo sicuro in `~/.local/share/corro/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Corro Code insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Corro Code come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Corro Code al tuo account Sentry.
