...
Practical reference for a future `packages/server` split after the corro
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/corro`.
...
Until enough shared service code lives outside `packages/corro`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/corro`

It should not import `packages/corro` services while `packages/corro`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on corro-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/corro` implementation details.
