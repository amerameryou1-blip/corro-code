...
Med Corro Code kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Corro Code vårt eget `corro`-tema.
...
Corro Code støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/corro/themes/*.json` eller `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.corro/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.corro/themes/*.json`
