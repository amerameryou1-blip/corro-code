...
corro ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\corro\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `corro --log-level DEBUG`.

---

## Depolama

corro, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/corro/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\corro` yapistirin
...
corro Desktop arka planda yerel bir corro sunucusu (`corro-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `corro` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/corro/corro.jsonc` (veya `~/.config/corro/corro.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/corro/corro.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\corro\corro.jsonc` yapistirin
...
corro diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/corro/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\corro\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), corro'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. corro Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/corro`
- **Linux**: `~/.cache/corro` dizinini silin (veya `rm -rf ~/.cache/corro` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\corro` yapistirin

3. corro Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

corro Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`corro.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta corro Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, corro ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

corro Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda corro icin bildirimler etkinse
...
1. corro Desktop'u kapatin
2. Su dosyalari bulun ve silin (corro Desktop uygulama veri dizininde yer alirlar):

- `corro.settings.dat` (desktop varsayilan sunucu URL'si)
- `corro.global.dat` ve `corro.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
corro ile ilgili bir sorun yasiyorsaniz:
...
3. `corro upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `corro models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\corro`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. corro, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\corro`

2. En guncel provider paketlerini yeniden kurmak icin corro'u yeniden baslatin

Bu, corro'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
corro, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
