...
Avec Corro Code, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Corro Code utilise notre propre thème `corro`.
...
Corro Code est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Corro Code corresponde à l'apparence de leur terminal
...
Corro Code prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/corro/themes/*.json` ou `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.corro/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.corro/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/corro/themes
vim ~/.config/corro/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .corro/themes
vim .corro/themes/my-theme.json
```
...
{
  "$schema": "https://corrocode.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
