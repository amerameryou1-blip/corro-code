...
Corro Code Zen은 Corro Code 팀이 OpenCode와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).

:::tip
처음이라면 Corro Code Zen으로 시작하는 것이 좋습니다.
:::

1. TUI에서 `/connect` 명령을 실행하고 `Corro Code Zen`을 선택한 뒤, [corrocode.dev/auth](https://corrocode.dev/zen)로 이동합니다.
...
Corro Code Go는 Corro Code 팀이 테스트하고 검증하여 OpenCode와 잘 작동하는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있는 저렴한 구독 요금제입니다.

1. TUI에서 `/connect` 명령을 실행하고 `Corro Code Go`를 선택한 뒤 [corrocode.dev/auth](https://corrocode.dev/zen)로 이동하십시오.
...
OpenCode를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `corro.json` 파일에 다음을 추가하십시오.
...
:::tip
Ollama는 OpenCode에 대해 자동으로 구성될 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://docs.ollama.com/integrations/corro)를 참조하십시오.
:::
