...
Sie können externe Tools zu Corro Code hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Corro Code unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Corro Code Config](https://corrocode.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/corro`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Corro Code übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Corro Code Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Corro Code beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `corro mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Corro Code die Token sicher in `~/.local/share/corro/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Corro Code verfügbar. Sie können sie also wie jedes andere Tool über die Corro Code-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Corro Code mit Ihrem Sentry-Konto zu verbinden.
