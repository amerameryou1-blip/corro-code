---
title: Plugins
description: Escreva seus próprios plugins para estender o corro.
---

Plugins permitem que você estenda o corro conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do corro.
...
- `.corro/plugins/` - Plugins em nível de projeto
- `~/.config/corro/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/corro/node_modules/`.
...
1. Configuração global (`~/.config/corro/corro.json`)
2. Configuração do projeto (`corro.json`)
3. Diretório de plugins global (`~/.config/corro/plugins/`)
4. Diretório de plugins do projeto (`.corro/plugins/`)
...
O corro executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do corro para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o corro.
...
:::note
Se você estiver usando o aplicativo desktop corro, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o corro de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao corro:
...
O helper `tool` cria uma ferramenta personalizada que o corro pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o corro junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://corrocode.dev/docs/sdk) para detalhes.
