...
Da biste otklonili probleme s Corro Code, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\corro\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `corro --log-level DEBUG`.

---

## Pohrana

Corro Code pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/corro/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\corro`
...
Corro Code Desktop pokreće lokalni Corro Code server (`corro-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Corro Code` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/corro/corro.jsonc` (ili `~/.config/corro/corro.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/corro/corro.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\corro\corro.jsonc`
...
Corro Code također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/corro/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\corro\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Corro Code mogao ponovo izgraditi.

1. Potpuno zatvorite Corro Code Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/corro`
- **Linux**: obrišite `~/.cache/corro` (ili pokrenite `rm -rf ~/.cache/corro`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\corro`

3. Ponovo pokrenite Corro Code Desktop.

---

### Rješavanje problema sa vezom na serveru

Corro Code Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `corro.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Corro Code Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Corro Code karakteristikama.

---

### Obavještenja se ne prikazuju

Corro Code Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Corro Code u postavkama vašeg OS-a, i
...
1. Zatvorite Corro Code Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Corro Code Desktop aplikacije):

- `corro.settings.dat` (URL zadanog servera za desktop)
- `corro.global.dat` i `corro.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Corro Code:
...
Da saznate kojim modelima imate pristup, pokrenite `corro models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\corro`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Corro Code dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\corro`

2. Ponovo pokrenite Corro Code da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Corro Code da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Corro Code će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
