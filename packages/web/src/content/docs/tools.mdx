...
Tools allow the LLM to perform actions in your codebase. Corro Code comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Corro Code.
...
:::note
This tool is only available when using the Corro Code or Corro Code Go provider, or when either the `OPENCODE_ENABLE_EXA` or `OPENCODE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Corro Code:

```bash
OPENCODE_ENABLE_EXA=1 corro
# or
OPENCODE_ENABLE_PARALLEL=1 corro
```
