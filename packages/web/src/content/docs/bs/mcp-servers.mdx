...
Možete dodati vanjske alate u Corro Code koristeći _Model Context Protocol_, ili MCP. Corro Code podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Corro Code Config](https://corrocode.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/corro`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Corro Code automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Corro Code će:
...
Ako server zahtijeva autentifikaciju, Corro Code će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `corro mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Corro Code sigurno čuva tokene u `~/.local/share/corro/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Corro Code, zajedno s ugrađenim alatima. Možete njima upravljati kroz Corro Code konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Corro Code sa vašim Sentry nalogom.
