...
Existem dois tipos de agentes no corro; agentes primários e subagentes.
...
corro vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Corro Code vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Corro Code vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Corro Code, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `corro.json`:
...
- Global: `~/.config/corro/agents/`
- Por projeto: `.corro/agents/`

```markdown title="~/.config/corro/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o corro usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do corro quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do corro usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Corro Code Zen](/docs/zen), você usaria `corro/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="corro.json" {3-6,9-12}
{
  "$schema": "https://corrocode.dev/config.json",
  "tools": {
    "write": true,
...
```json title="corro.json" {3-5,8-10}
{
  "$schema": "https://corrocode.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `corro models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/corro).
:::
...
```markdown title="~/.config/corro/agents/docs-writer.md"
---
...
```markdown title="~/.config/corro/agents/security-auditor.md"
---
