...
Możesz dodać dodatkowe informacje do corro za pomocą _Model Context Protocol_, w skrócie MCP. corro obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [corro Config](https://corrocode.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/corro`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
corro automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, corro:
...
Jeśli serwer wymaga uwierzytelnienia, corro poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `corro mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji corro bezpieczne przechowa tokeny w `~/.local/share/corro/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w corro, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie corro, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć corro z kontem Sentry.
