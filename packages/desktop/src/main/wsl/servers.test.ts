...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Corro Code update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Corro Code", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("ignores stale background Corro Code checks after removing a WSL server", async () => {
  persistedServers = []
...
test("ignores stale startup Corro Code checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Corro Code", async () => {
  persistedServers = []
...
test("does not check Corro Code in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
