---
title: Plugins
description: Écrivez vos propres plugins pour étendre Corro Code.
---

Les plugins vous permettent d'étendre Corro Code en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Corro Code.
...
- `.corro/plugins/` - Plugins au niveau du projet
- `~/.config/corro/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/corro/node_modules/`.
...
1. Configuration globale (`~/.config/corro/corro.json`)
2. Configuration du projet (`corro.json`)
3. Répertoire global des plugins (`~/.config/corro/plugins/`)
4. Répertoire des plugins du projet (`.corro/plugins/`)
...
Corro Code exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK corro pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre corro.
...
:::note
Si vous utilisez l'application de bureau Corro Code, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez corro de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à corro :
...
L'assistant `tool` crée un outil personnalisé que corro peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour corro aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://corrocode.dev/docs/sdk) pour plus de détails.
