---
title: Server
description: Nutze den corro-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `corro serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein corro-Client nutzen kann.
...
Der Benutzername ist standardmaessig `corro`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `corro serve` und `corro web`.
...
Wenn du `corro` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den corro-Server, um corro programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `corro serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `corro serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Corro Code-[IDE](/docs/ide)-Plugins.
...
Der corro-Server stellt folgende APIs bereit.
