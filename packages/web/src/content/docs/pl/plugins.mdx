---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć corro.
---

Wtyczki umożliwiające wyłączenie corro poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania corro.
...
- `.corro/plugins/` - Wtyczki na poziomie projektu
- `~/.config/corro/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/corro/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/corro/corro.json`)
2. Konfiguracja projektu (`corro.json`)
3. Globalny katalog wtyczek (`~/.config/corro/plugins/`)
4. Katalog wtyczek projektu (`.corro/plugins/`)
...
corro uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z corro do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do corro.
...
:::note
Jeśli korzystasz z aplikacji komputerowej corro, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj corro czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do corro:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod corro. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://corrocode.dev/docs/sdk).
