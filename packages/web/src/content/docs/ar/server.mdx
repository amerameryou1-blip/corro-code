---
title: الخادم
description: تفاعل مع خادم corro عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `corro serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل corro استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `corro.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `corro`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `corro serve` و `corro web`.
...
عند تشغيل `corro` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم corro للتفاعل مع corro برمجيا.
:::

تتيح هذه البنية لـ corro دعم عدة عملاء وتمكّنك من التفاعل مع corro برمجيا.

يمكنك تشغيل `corro serve` لبدء خادم مستقل. إذا كانت واجهة corro في terminal (TUI)
قيد التشغيل، فسيبدأ `corro serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Corro Code لـ [IDE](/docs/ide).
...
يعرض خادم corro واجهات API التالية.
