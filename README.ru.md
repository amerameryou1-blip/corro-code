...
# Менеджеры пакетов
npm i -g @corro-code@latest        # или bun/pnpm/yarn
scoop install corro             # Windows
choco install corro             # Windows
brew install anomalyco/tap/corro # macOS и Linux (рекомендуем, всегда актуально)
brew install corro              # macOS и Linux (официальная формула brew, обновляется реже)
sudo pacman -S corro            # Arch Linux (Stable)
paru -S corro-bin               # Arch Linux (Latest from AUR)
mise use -g corro               # любая ОС
nix run nixpkgs#corro           # или github:anomalyco/corro для самой свежей ветки dev
```
...
Corro Code также доступен как десктопное приложение. Скачайте его со [страницы релизов](https://github.com/anomalyco/corro/releases) или с [corrocode.dev/download](https://corrocode.dev/download).
...
4. `$HOME/.corro/bin` - Fallback по умолчанию
...
В Corro Code есть два встроенных агента, между которыми можно переключаться клавишей `Tab`.
...
Подробнее об [agents](https://corrocode.dev/docs/agents).

### Документация

Больше информации о том, как настроить Corro Code: [**наши docs**](https://corrocode.dev/docs).

### Вклад

Если вы хотите внести вклад в Corro Code, прочитайте [contributing docs](./CONTRIBUTING.md) перед тем, как отправлять pull request.

### Разработка на базе Corro Code

Если вы делаете проект, связанный с Corro Code, и используете "corro" как часть имени (например, "corro-dashboard" или "corro-mobile"), добавьте примечание в README, чтобы уточнить, что проект не создан командой Corro Code и не аффилирован с нами.

---

**Присоединяйтесь к нашему сообществу** [Discord](https://discord.gg/corro) | [X.com](https://x.com/corro)
