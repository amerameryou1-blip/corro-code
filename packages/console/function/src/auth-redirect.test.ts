...
describe("authorization redirect validation", () => {
  test("allows registered OpenCode callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://corrocode.dev/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.corrocode.dev/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://corrocode.dev/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://corrocode.dev.evil.example/callback")).toBe(false)
