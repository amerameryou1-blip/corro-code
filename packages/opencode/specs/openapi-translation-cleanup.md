...
Trim `packages/corro/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/corro/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/corro`.
- `[x]` `bun typecheck` from `packages/corro`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/corro/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/corro`.
- `[x]` `bun dev generate > /tmp/corro-openapi.json` from `packages/corro`.
...
- Edit `packages/corro/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/corro-openapi.json` from `packages/corro`.
...
- Check source schemas in files like `packages/corro/src/session/schema.ts`, `packages/corro/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/corro/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/corro/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
