...
Both `action` and `resource` use corro's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.corro` files is intentionally deferred until `.corro` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether corro permits an operation through its managed execution paths.
