---
title: دعم ACP
description: استخدم Corro Code في أي محرر متوافق مع ACP.
---

يدعم Corro Code بروتوكول [Agent Client Protocol](https://agentclientprotocol.com) أو (ACP)، مما يتيح لك استخدامه مباشرة في المحررات وبيئات التطوير المتكاملة (IDEs) المتوافقة.
...
لاستخدام Corro Code عبر ACP، اضبط محررك ليشغّل الأمر `corro acp`.

يشغّل هذا الأمر Corro Code كعملية فرعية متوافقة مع ACP تتواصل مع محررك عبر JSON-RPC باستخدام stdio.
...
ثبّت Corro Code من [سجل ACP في Zed](https://zed.dev/docs/ai/external-agents#registry) عبر تشغيل `zed: acp registry` من لوحة الأوامر.

لاستخدام ملف Corro Code تنفيذي مخصص بدلاً من ذلك، أضفه إلى إعدادات [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
لفتحه، اختر الوكيل الجديد 'Corro Code' من محدد الوكلاء في AI Chat.
...
لاستخدام Corro Code كوكيل ACP في [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)، أضف التالي إلى إعدادات Neovim:
...
يضبط هذا الإعداد CodeCompanion لاستخدام Corro Code كوكيل ACP للدردشة.
...
يعمل Corro Code عبر ACP بالطريقة نفسها التي يعمل بها في terminal. جميع الميزات مدعومة:
...
- خوادم MCP المضبوطة في إعدادات Corro Code
