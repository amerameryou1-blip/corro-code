---
title: Servidor
description: Interactuar con el servidor corro a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `corro serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente corro puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `corro.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `corro`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `corro serve` como a `corro web`.
...
Cuando ejecuta `corro`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor corro para interactuar con corro mediante programación.
:::

Esta arquitectura permite que corro admita múltiples clientes y le permite interactuar con corro mediante programación.

Puede ejecutar `corro serve` para iniciar un servidor independiente. Si tienes el
corro TUI ejecutándose, `corro serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Corro Code [IDE](/docs/ide).
...
El servidor corro expone las siguientes API.
