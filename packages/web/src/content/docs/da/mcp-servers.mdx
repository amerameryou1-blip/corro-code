...
Du kan tilføje eksterne værktøjer til Corro Code ved hjælp af _Model Context Protocol_ eller MCP. Corro Code understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Corro Code Config](https://corrocode.dev/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/corro`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Corro Code håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Corro Code:
...
Hvis serveren kræver godkendelse, vil Corro Code bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `corro mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Corro Code opbevare tokens sikkert i `~/.local/share/corro/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Corro Code-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Corro Code til din Sentry-konto.
