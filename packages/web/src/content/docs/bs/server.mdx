---
title: Server
description: Komunicirajte s corro serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `corro serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji corro klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `corro.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `corro`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `corro serve` i za `corro web`.
...
Kada pokrenete `corro`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite corro server za programsku interakciju sa corro-om.
:::

Ova arhitektura omogucava corro podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `corro serve` da startate standalone server. Ako je corro TUI vec pokrenut, `corro serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Corro Code [IDE](/docs/ide) pluginovi.
...
corro server izlaže sljedece API-je.
