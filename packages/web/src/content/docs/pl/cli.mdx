---
title: CLI
description: Opcje i polecenia wiersza poleceń Corro Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Corro Code domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
corro
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Corro Code programowo.
...
Uruchamia interfejs terminalowy (TUI) Corro Code.
...
Interfejs CLI Corro Code zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Corro Code.
...
Dołącz terminal do już działającego serwera backendu Corro Code uruchomionego za pomocą `serve` lub `web`.

```bash
corro attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Corro Code. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `corro`) |
...
Corro Code integruje się z wieloma dostawcami. Możesz użyć `corro auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

Podczas uruchamiania Corro Code ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Corro Code.
...
Uruchom Corro Code w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `corro serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Corro Code (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `corro`) |
...
Uruchom serwer Corro Code (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
corro serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Corro Code bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `corro`).
...
Lista wszystkich sesji Corro Code.
...
Pokaż statystyki wykorzystania tokenów i sesji Corro Code.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Corro Code.
...
Uruchom serwer Corro Code z interfejsem internetowym.

```bash
corro web
```

Uruchamia to serwer HTTP i udostępnia Corro Code przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `corro`).
...
Odinstaluj Corro Code i usuń wszystkie powiązane pliki.
...
Aktualizuj Corro Code do najnowszej lub określonej wersji.
...
Interfejs CLI Corro Code przyjmuje następujące flagi globalne dla każdego polecenia.
...
Corro Code można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `corro`)    |
