---
title: CLI
description: Corro Code CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Corro Code CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
corro
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Corro Code programmatisk.
...
Start Corro Code-terminalbrukergrensesnittet.
...
Administrer agenter for Corro Code.
...
Koble en terminal til en allerede kjørende Corro Code-backend-server startet via `serve` eller `web` kommandoer.

```bash
corro attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Corro Code-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `corro`) |
...
Corro Code drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `corro auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

Når Corro Code starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Corro Code.
...
Kjør Corro Code i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `corro serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Corro Code-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `corro`) |
...
Start en headless Corro Code-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
corro serve
```

Dette starter en HTTP-server som gir API tilgang til Corro Code-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `corro`).
...
List opp alle Corro Code-økter.
...
Vis tokenbruk og kostnadsstatistikk for Corro Code-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Corro Code share-URL.
...
Start en headless Corro Code-server med et webgrensesnitt.

```bash
corro web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Corro Code via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `corro`).
...
Avinstaller Corro Code og fjern alle relaterte filer.
...
Oppdaterer Corro Code til siste versjon eller en spesifikk versjon.
...
Corro Code CLI bruker følgende globale flagg.
...
Corro Code kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `corro`) |
