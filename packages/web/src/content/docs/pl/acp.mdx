---
title: ACP
description: Używanie Corro Code z protokołem Agent Client Protocol (ACP).
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Corro Code obsługuje [Agent Client Protocol](https://agentclientprotocol.com) (ACP), który umożliwia korzystanie z Corro Code bezpośrednio w edytorach i środowiskach IDE.
...
Aby używać Corro Code przez ACP, skonfiguruj swój edytor tak, aby uruchamiał polecenie `corro acp`.

Uruchamia to Corro Code jako podproces zgodny z ACP, który komunikuje się z edytorem za pomocą JSON-RPC przez stdio.
...
Zainstaluj Corro Code z [rejestru ACP Zed](https://zed.dev/docs/ai/external-agents#registry), uruchamiając `zed: acp registry` w palecie poleceń.

Aby zamiast tego użyć niestandardowego pliku wykonywalnego Corro Code, dodaj go do konfiguracji [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
Następnie wybierz „Corro Code” w selektorze agentów AI Chat.
...
Aby używać Corro Code jako agenta ACP w [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), dodaj następującą konfigurację do Neovim:
...
Ta konfiguracja sprawia, że CodeCompanion używa Corro Code jako agenta ACP do czatu.
...
Corro Code działa przez ACP tak samo, jak w terminalu. Obsługiwane są wszystkie główne funkcje:
...
- Serwery MCP zdefiniowane w konfiguracji Corro Code
