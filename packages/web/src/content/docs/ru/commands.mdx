...
Создайте `.corro/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию corro или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем corro [config](/docs/config):
...
- Глобальный: `~/.config/corro/commands/`
- Для каждого проекта: `.corro/commands/`
...
corro включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
