...
The corro server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@corro-code/llm` tool loop after replacing its
...
- plugins get global "corro" instance like in that post i showed
- corro instance has stuff like `corro.session.prompt()` or
  `corro.tool.register({...})`
...
- decide whether to preserve deprecated `@corro-code/llm` orchestration exports
