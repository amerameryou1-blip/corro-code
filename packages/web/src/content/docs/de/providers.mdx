---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Corro Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Corro Code verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Corro Code-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Corro Code anpassen
config.
...
Corro Code Zen ist eine Liste von Modellen, die vom Corro Code-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Corro Code funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Corro Code Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in Corro Code und ist völlig optional.

---

## Corro Code Go

Corro Code Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Corro Code-Team getestet und verifiziert wurden, dass sie gut mit Corro Code funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Corro Code Go` und gehen Sie zu [corrocode.dev/auth](https://corrocode.dev/zen).
...
So verwenden Sie Amazon Bedrock mit Corro Code:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Corro Code ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `corro.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Corro Code wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können corro so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Corro Code ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Corro Code-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Corro Code`, Bereiche: `api`
...
um das Token im Corro Code-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Corro Code verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Corro Code
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`corro.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [corro-gitlab-auth](https://www.npmjs.com/package/corro-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `corro.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Corro Code:
...
So verwenden Sie Google Vertex AI mit Corro Code:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Corro Code ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Corro Code konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [corro-helicone-session](https://github.com/H2Shami/corro-helicone-session)-Plugin, um jede Corro Code-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Corro Code-Konversation als separate Sitzung aufgeführt.
...
Sie können Corro Code für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Corro Code über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Corro Code über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Corro Code konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/corro).
:::
...
So verwenden Sie Ollama Cloud mit Corro Code:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Corro Code.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Corro Code verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Corro Code Zen ist eine Liste getesteter und verifizierter Modelle, die vom Corro Code-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Corro Code Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Corro Code Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Corro Code API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Corro Code-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Corro Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Corro Code:
...
Sie können Modelle auch über Ihre Corro Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Corro Code verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `corro.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Corro Code erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `corro auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Corro Code-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Corro Code-Konfiguration übereinstimmt.
