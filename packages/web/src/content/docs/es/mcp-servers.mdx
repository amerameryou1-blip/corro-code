...
Puede agregar herramientas externas a Corro Code utilizando el _Model Context Protocol_ o MCP. Corro Code admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Corro Code Config](https://corrocode.dev/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/corro`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Corro Code maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Corro Code:
...
Si el servidor requiere autenticación, Corro Code le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `corro mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Corro Code almacenará los tokens de forma segura en `~/.local/share/corro/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Corro Code, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Corro Code como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Corro Code a su cuenta Sentry.
