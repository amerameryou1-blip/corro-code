---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Corro Code zu erweitern.
---

Mit Plugins können Sie Corro Code erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Corro Code zu ändern.
...
- `.corro/plugins/` – Plugins auf Projektebene
- `~/.config/corro/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/corro/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/corro/corro.json`)
2. Projektkonfiguration (`corro.json`)
3. Globales Plugin-Verzeichnis (`~/.config/corro/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.corro/plugins/`)
...
Corro Code führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Corro Code-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Corro Code erweitern können.
...
:::note
Wenn Sie die Corro Code-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Corro Code `.env`-Dateien liest:
...
Plugins können Corro Code auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Corro Code aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Corro Code zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://corrocode.dev/docs/sdk).
