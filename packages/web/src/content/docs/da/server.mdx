---
title: Server
description: Interager med corro-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `corro serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en corro-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `corro.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `corro`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `corro serve` og `corro web`.
...
Når du kører `corro` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug corro-serveren til at interagere med corro programmatisk.
:::

Denne arkitektur lader corro understøtte flere klienter og lader dig interagere med corro programmatisk.

Du kan køre `corro serve` for at starte en selvstændig server. Hvis du har
corro TUI kørende, vil `corro serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Corro Code [IDE](/docs/ide) plugins.
...
Corro Code-serveren viser følgende API'er.
