...
Corro Code Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, Corro Code'daki diğer sağlayıcılar gibi çalışır. Corro Code Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Corro Code'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Corro Code Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Corro Code Go, Corro Code'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Corro Code Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Corro Code Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Corro Code Go'ya abone olabilir.
:::
...
Corro Code Go aşağıdaki limitleri içerir:
...
Corro Code yapılandırmanızdaki [model id](/docs/config/#models) formatı `corro-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `corro-go/kimi-k3` kullanmalısınız.
...
Corro Code Go'yu şu amaçlarla oluşturduk:
...
4. Corro Code ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
