...
You can add external tools to Corro Code using the _Model Context Protocol_, or MCP. Corro Code supports both local and remote servers.
...
You can define MCP servers in your [Corro Code Config](https://corrocode.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/corro` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Corro Code automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Corro Code will:
...
If the server requires authentication, Corro Code will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `corro mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Corro Code will store the tokens securely in `~/.local/share/corro/mcp-auth.json`.
...
Your MCPs are available as tools in Corro Code, alongside built-in tools. So you can manage them through the Corro Code config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Corro Code to your Sentry account.
