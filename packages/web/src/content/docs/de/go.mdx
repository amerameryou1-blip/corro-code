...
Corro Code Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Corro Code. Du abonnierst Corro Code Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Corro Code zu verwenden.
...
Corro Code Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

Corro Code Go funktioniert wie jeder andere Provider in Corro Code.

1. Du meldest dich bei **<a href={console}>Corro Code Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Corro Code Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Corro Code Go abonnieren.
:::
...
Corro Code Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Corro Code Config verwendet das Format `corro-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `corro-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Corro Code Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Corro Code zu nutzen.
