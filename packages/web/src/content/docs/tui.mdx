---
title: TUI
description: Using the Corro Code terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Corro Code provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Corro Code starts the TUI for the current directory.
...
When using the Corro Code TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Corro Code. Allows you to select from available providers and add their API keys.
...
Exit Corro Code. _Aliases_: `/quit`, `/q`
...
This is separate from `corro.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Corro Code waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `corro.default`.
