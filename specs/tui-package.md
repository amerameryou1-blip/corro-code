...
Move the canonical Corro Code terminal application from
`packages/corro/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/corro`,
`packages/cli`, or `@corro-code/core`.

The SDK is the TUI's Corro Code boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/corro`.
- Do not replace private `packages/corro` imports with `@corro-code/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@corro-code/tui`.
...
- Corro Code domain data displayed by the TUI
...
```text
packages/corro/src/cli/cmd/tui
```

Its private dependency on `packages/corro` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/corro/src/*`.
...
Status: Completed. The private `@corro-code/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@corro-code/tui`.
...
- The package has no dependency on `corro`, `@corro-code/cli`, or
  `@corro-code/core`.
...
primitives now live in `@corro-code/tui`. The legacy host consumes explicit
...
and build inputs through `@corro-code/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Corro Code globals or process state;
...
Replace process-global Corro Code state with resolved TUI inputs.
...
`@corro-code/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/corro` initially.
...
canonical in `@corro-code/tui`. Configured references resolve through the new
...
adapters rather than Corro Code domain implementations.

Make the SDK the only Corro Code domain boundary used by the TUI.
...
- Domain-facing TUI code imports Corro Code data and operations only from
  `@corro-code/sdk`.
...
`@corro-code/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/corro`.
...
- The package has no imports from `packages/corro`, `packages/cli`, or
  `@corro-code/core`.
...
public `@corro-code/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/corro`.
- Keep the legacy embedded worker and server startup in `packages/corro`.
- Change those adapters to load config, create transport inputs, and call the
  public `@corro-code/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/corro`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@corro-code/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@corro-code/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Corro Code domain state.
...
cd packages/corro && bun typecheck
...
rg '@corro-code/core|packages/corro|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/corro/src packages/corro/test
```
...
- Build the current-platform `packages/corro` binary.
