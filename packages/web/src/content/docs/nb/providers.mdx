---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Corro Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Corro Code bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Corro Code-konfigurasjonen.
...
i `~/.local/share/corro/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Corro Code
konfig.
...
Corro Code Zen er en liste over modeller levert av Corro Code-teamet som har vært
testet og verifisert for å fungere godt med Corro Code. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Corro Code Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Corro Code Zen` og gå til [corrocode.dev/auth](https://corrocode.dev/zen).
...
Det fungerer som alle andre leverandører i Corro Code og er helt valgfritt å bruke.

---

## Corro Code Go

Corro Code Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Corro Code-teamet som har vært
testet og verifisert for å fungere godt med Corro Code.

1. Kjør kommandoen `/connect` i TUI, velg `Corro Code Go`, og gå til [corrocode.dev/auth](https://corrocode.dev/zen).
...
Slik bruker du Amazon Bedrock med Corro Code:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Corro Code:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `corro.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i corro støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere corro til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Corro Code skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Corro Code-konfigurasjonen.
...
for å lagre token i Corro Code auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Corro Code bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Corro Code
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`corro.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [corro-gitlab-auth](https://www.npmjs.com/package/corro-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `corro.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Corro Code:
...
Slik bruker du Google Vertex AI med Corro Code:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Corro Code.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Corro Code, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [corro-helicone-session](https://github.com/H2Shami/corro-helicone-session) plugin for automatisk å logge hver Corro Code-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Corro Code-samtale oppført som en separat økt.
...
Du kan konfigurere Corro Code for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Corro Code for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Corro Code for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Corro Code. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/corro) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Corro Code:
...
3. Kopier API-nøkkelen for bruk i Corro Code.
...
6. **Viktig**: Før du bruker skymodeller i Corro Code, må du hente modellinformasjonen lokalt:
...
Corro Code Zen er en liste over testede og verifiserte modeller levert av Corro Code-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Corro Code Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Corro Code Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Corro Code API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Corro Code-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Corro Code-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Corro Code:
...
Du kan også tilpasse modeller gjennom Corro Code-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Corro Code. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `corro.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Corro Code forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `corro auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Corro Code-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Corro Code-konfigurasjonen.
