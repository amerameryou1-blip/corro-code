# Remove `packages/corro/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/corro/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the corro legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/corro/src/storage/db.ts` are concentrated in 22 source files:

- `packages/corro/src/account/repo.ts`
- `packages/corro/src/cli/cmd/db.ts`
- `packages/corro/src/cli/cmd/import.ts`
- `packages/corro/src/cli/cmd/stats.ts`
- `packages/corro/src/control-plane/workspace.ts`
- `packages/corro/src/index.ts`
- `packages/corro/src/node.ts`
- `packages/corro/src/permission/index.ts`
- `packages/corro/src/project/project.ts`
- `packages/corro/src/server/projectors.ts`
- `packages/corro/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/corro/src/server/shared/fence.ts`
- `packages/corro/src/session/message-v2.ts`
- `packages/corro/src/session/projectors.ts`
- `packages/corro/src/session/prompt.ts`
- `packages/corro/src/session/session.ts`
- `packages/corro/src/session/todo.ts`
- `packages/corro/src/share/share-next.ts`
- `packages/corro/src/storage/db.ts`
- `packages/corro/src/sync/index.ts`
- `packages/corro/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy corro database wrapper; `packages/corro/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the corro wrapper.
...
Status: Completed. `SyncEvent` and the corro projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy corro database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy corro database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy corro database wrapper.
...
All migration groups are complete or superseded. `packages/corro/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No corro data-migration group remains.

The previous corro `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate corro data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/corro`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/corro/src`
- `bun typecheck` from `packages/corro`
- Relevant package tests from `packages/corro`, not the repo root
