---
title: CLI
description: corro CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

corro CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
corro
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, corro ile programlı olarak etkileşim kurmanıza olanak tanır.
...
corro terminal kullanıcı arayüzünü başlatır.
...
corro CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

corro için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir corro arka uç sunucusuna bir terminal ekleyin.

```bash
corro attach [url]
```

Bu, TUI öğesinin uzak bir corro arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `corro`) |
...
corro, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `corro auth login` kullanabilirsiniz. Bu `~/.local/share/corro/auth.json`'de saklanıyor.

```bash
corro auth login
```

corro başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları corro'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek corro'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `corro serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir corro sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `corro`) |
...
API erişimi için headless bir corro sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
corro serve
```

Bu, TUI arayüzü olmadan corro işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `corro` şeklindedir).
...
Tüm corro oturumlarını listeleyin.
...
corro oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya corro paylaşımından URL oturum verilerini içe aktarın.

```bash
corro import <file>
```

Yerel bir dosyadan veya corro paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir corro sunucusu başlatın.

```bash
corro web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla corro'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `corro` şeklindedir).
...
corro'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
corro'u en son sürüme veya belirli bir sürüme günceller.
...
corro CLI aşağıdaki global bayrakları destekler.
...
corro ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `corro`) |
