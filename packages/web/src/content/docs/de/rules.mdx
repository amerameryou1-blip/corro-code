---
title: Regeln
description: Lege eigene Anweisungen fuer corro fest.
---

Du kannst corro ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in corro den Befehl `/init` aus.
...
So kann corro sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht corro sie zu erweitern.
...
corro kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/corro/AGENTS.md` speichern.
Diese gelten in allen corro-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Corro Code dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/corro/AGENTS.md` existiert)
...
Beim Start sucht corro in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/corro/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `corro.json` oder in `~/.config/corro/corro.json` angeben.
...
corro parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit corro.json

Empfohlen ist das `instructions`-Feld in der `corro.json`:
...
Du kannst corro explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `corro.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
