...
Per diagnosticare problemi con Corro Code, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\corro\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `corro --log-level DEBUG`.

---

## Archiviazione

corro salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/corro/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\corro`
...
Corro Code Desktop esegue in background un server locale di Corro Code (il sidecar `corro-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Corro Code` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/corro/corro.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\corro\corro.jsonc`
...
Corro Code puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/corro/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\corro\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Corro Code possa ricostruirla.

1. Chiudi completamente Corro Code Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/corro`
- **Linux**: elimina `~/.cache/corro` (oppure esegui `rm -rf ~/.cache/corro`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\corro`

3. Riavvia Corro Code Desktop.

---

### Risolvi problemi di connessione al server

Corro Code Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `corro.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, Corro Code Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Corro Code.

---

### Notifiche non visualizzate

Corro Code Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Corro Code nelle impostazioni del sistema operativo, e
...
1. Chiudi Corro Code Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Corro Code Desktop):

- `corro.settings.dat` (desktop default server URL)
- `corro.global.dat` e `corro.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Corro Code:
...
Per capire a quali modelli hai accesso, esegui `corro models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\corro`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. corro installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\corro`

2. Riavvia corro per reinstallare i pacchetti provider piu recenti

Questo forzera corro a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
corro rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
