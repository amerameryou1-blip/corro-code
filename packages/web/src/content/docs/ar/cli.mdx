---
title: CLI
description: أوامر وخيارات CLI في Corro Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Corro Code افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
corro
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Corro Code برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Corro Code.
...
يتضمن CLI الخاص بـ Corro Code أيضا الأوامر التالية.

---

### agent

إدارة وكلاء Corro Code.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Corro Code قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
corro attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Corro Code تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `corro`) |
...
يعتمد Corro Code على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `corro auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

عند تشغيل Corro Code يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Corro Code.
...
تشغيل corro في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `corro serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم corro قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `corro`) |
...
بدء خادم Corro Code بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
corro serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف corro دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `corro`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Corro Code لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Corro Code.

```bash
corro import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Corro Code.
...
بدء خادم Corro Code بدون واجهة مع واجهة ويب.

```bash
corro web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Corro Code عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `corro`).
...
إلغاء تثبيت Corro Code وإزالة جميع الملفات ذات الصلة.
...
تحديث corro إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر corro الخيارات العامة التالية.
...
يمكن تهيئة Corro Code باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `corro`) |
