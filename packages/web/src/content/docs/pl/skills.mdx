...
Umiejętności agenta pozwalają corro odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
corro przeszukuje te lokalizacje:

- Project config: `.corro/skills/<name>/SKILL.md`
- Global config: `~/.config/corro/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu corro przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.corro/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/corro/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.corro/skills/git-release/SKILL.md` w ten sposób:
...
corro wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `corro.json`:
...
**Dla agentów wbudowanych** (w `corro.json`):
