...
Le skill degli agenti permettono a Corro Code di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Corro Code cerca in queste posizioni:

- Config di progetto: `.corro/skills/<name>/SKILL.md`
- Config globale: `~/.config/corro/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Corro Code risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.corro/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/corro/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.corro/skills/git-release/SKILL.md` cosi':
...
Corro Code elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `corro.json`:
...
**Per agenti integrati** (in `corro.json`):
