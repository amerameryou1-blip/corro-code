...
const defaultServer = "https://corrocode.dev/console"
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Corro Code provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
