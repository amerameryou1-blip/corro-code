...
مع Corro Code يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Corro Code سمتنا `corro`.
...
يأتي Corro Code مع عدة سمات مدمجة.
...
- يريدون أن يطابق Corro Code مظهر terminal لديهم
...
يدعم Corro Code نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/corro/themes/*.json` أو `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.corro/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.corro/themes/*.json`
