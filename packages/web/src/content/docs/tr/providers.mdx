---
title: Sağlayıcılar
description: corro'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

corro, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı corro yapılandırmanızda yapılandırın.
...
`~/.local/share/corro/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları corro'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Corro Code Zen, corro ekibi tarafından sağlanan modellerin bir listesidir.
corro ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Corro Code Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, corro'u seçin ve [corrocode.dev/auth](https://corrocode.dev/auth)'ye gidin.
...
corro'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Corro Code Go

Corro Code Go, corro ile iyi çalıştığı test edilmiş ve doğrulanmış, corro ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Corro Code Go`'yu seçin ve [corrocode.dev/auth](https://corrocode.dev/zen) adresine gidin.
...
Amazon Bedrock'u corro ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   corro'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `corro.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi corro'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
corro'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   corro'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
corro, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. corro'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`corro.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [corro-gitlab-auth](https://www.npmjs.com/package/corro-gitlab-auth) ana sayfasında.

##### Yapılandırma

`corro.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi corro'la kullanmak için:
...
Google Vertex AI'yi corro ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları corro çalıştırılırken ayarlayın.
...
Helicone'un corro aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her corro görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [corro-helicone-session](https://github.com/H2Shami/corro-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her corro görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
corro'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla corro'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla corro'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini corro için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/corro)'a bakın.
:::
...
Ollama Cloud'u corro ile kullanmak için:
...
3. corro'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: corro'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Corro Code Zen, corro ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Corro Code Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Corro Code Zen**'i arayın.

   ```txt
   /connect
   ```

3. corro API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   corro yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca corro yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri corro yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı corro ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `corro.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları corro'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `corro auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için corro yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, corro yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
