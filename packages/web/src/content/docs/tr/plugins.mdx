---
title: Eklentiler
description: Corro Code'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Corro Code'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Corro Code'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.corro/plugins/` - Proje düzeyinde eklentiler
- `~/.config/corro/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/corro/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/corro/corro.json`)
2. Project config (`corro.json`)
3. Global eklenti dizini (`~/.config/corro/plugins/`)
4. Proje eklenti dizini (`.corro/plugins/`)
...
Corro Code bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Corro Code SDK istemcisi.
...
Corro Code'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Corro Code masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Corro Code'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Corro Code'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Corro Code'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Corro Code'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://corrocode.dev/docs/sdk) bakın.
