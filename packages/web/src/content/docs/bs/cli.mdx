---
title: CLI
description: Corro Code CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Corro Code CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
corro
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Corro Code.
...
Pokrenite Corro Code terminalski korisnički interfejs.
...
Corro Code CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Corro Code agentima.
...
Priključite terminal na već pokrenut Corro Code backend server pokrenut putem `serve` ili `web` naredbi.

```bash
corro attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Corro Code backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `corro`) |
...
Corro Code pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `corro auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

Kada se Corro Code pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Corro Code.
...
Pokrenite Corro Code u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `corro serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Corro Code server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `corro`) |
...
Pokrenite Corro Code headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
corro serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Corro Code-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `corro`).
...
Upravljajte Corro Code sesijama.
...
Navedite sve Corro Code sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše Corro Code sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Corro Code dijeljenog URL-a.

```bash
corro import <file>
```

Možete uvesti iz lokalne datoteke ili Corro Code dijeljenog URL-a.
...
Pokrenite Corro Code headless server sa web interfejsom.

```bash
corro web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Corro Code-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `corro`).
...
Deinstalirajte Corro Code i uklonite sve povezane datoteke.
...
Ažurira Corro Code na najnoviju verziju ili određenu verziju.
...
Corro Code CLI prihvata sljedeće globalne zastavice.
...
Corro Code se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `corro`) |
