...
With Corro Code you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Corro Code uses our own `corro` theme.
...
Corro Code comes with several built-in themes.
...
- Want Corro Code to match their terminal's appearance
...
Corro Code supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/corro/themes/*.json` or `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.corro/themes/*.json`
4. **Current working directory** - Defined in `./.corro/themes/*.json`
