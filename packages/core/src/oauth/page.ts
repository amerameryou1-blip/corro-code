...
// etc.).
//
// The visual language mirrors the Corro Code app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Corro Code is now connected to ${escapeHtml(provider)}.` : "Corro Code is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Corro Code couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Corro Code couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Corro Code.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Corro Code</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Corro Code couldn't finish connecting to "+PROVIDER+"."):"Corro Code couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Corro Code."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Corro Code is now connected to "+PROVIDER+"."):"Corro Code is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Corro Code lockup — copper mark + wordmark text (replaces upstream pixel wordmark).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="Corro Code" role="img">
        <rect x="2" y="6" width="30" height="30" rx="8" fill="#b0662a" />
        <path d="M23.5 13.5c-1.4-1.2-3.2-2-5.2-2-4.4 0-8 3.6-8 8.5s3.6 8.5 8 8.5c2 0 3.8-.8 5.2-2l-2.3-2.6c-.8.7-1.8 1.1-2.9 1.1-2.5 0-4.5-2.1-4.5-5s2-5 4.5-5c1.1 0 2.1.4 2.9 1.1l2.3-2.6Z" fill="#fff" />
        <text x="40" y="29" font-family="system-ui, -apple-system, sans-serif" font-size="21" font-weight="700" fill="var(--oc-icon-strong)">Corro Code</text>
      </svg>`
