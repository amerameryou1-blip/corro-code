---
title: Support ACP
description: Utilisez Corro Code dans n’importe quel éditeur compatible ACP.
---

Corro Code prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Corro Code via ACP, configurez votre éditeur pour exécuter la commande `corro acp`.

La commande démarre Corro Code en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Installez Corro Code depuis le [registre ACP de Zed](https://zed.dev/docs/ai/external-agents#registry) en exécutant `zed: acp registry` dans la palette de commandes.

Pour utiliser plutôt un exécutable Corro Code personnalisé, ajoutez-le à votre configuration [Zed](https://zed.dev) (`~/.config/zed/settings.json`) :
...
Pour l'ouvrir, utilisez le nouvel agent « Corro Code » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Corro Code comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Corro Code comme agent ACP pour le chat.
...
Corro Code fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Corro Code
