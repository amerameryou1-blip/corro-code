...
Dzięki corro możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie corro używa naszego własnego motywu `corro`.
...
corro ma kilka wbudowanych motywów.
...
- Chcą, aby Corro Code pasował do wyglądu ich terminala
...
corro obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/corro/themes/*.json` lub `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.corro/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.corro/themes/*.json`
