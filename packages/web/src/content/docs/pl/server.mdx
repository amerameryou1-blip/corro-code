---
title: Serwer
description: Interakcja z serwerem corro za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `corro serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient corro.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `corro.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `corro` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `corro serve`, jak i `corro web`.
...
Kiedy uruchomisz `corro`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera corro do programowej interakcji z corro.
:::

Ta architektura umożliwia obsługę wielu klientów przez corro i programową interakcję z kodem otwartym.

Możesz uruchomić `corro serve`, aby uruchomić samodzielny serwer. Jeśli masz
corro TUI działa, `corro serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki corro [IDE](/docs/ide).
...
Serwer corro udostępnia następujące interfejsy API.
