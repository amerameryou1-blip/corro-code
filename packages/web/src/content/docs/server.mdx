---
title: Server
description: Interact with corro server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `corro serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an corro client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `corro.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `corro`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `corro serve` and `corro web`.
...
When you run `corro` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the corro server to interact with corro programmatically.
:::

This architecture lets corro support multiple clients and allows you to interact with corro programmatically.

You can run `corro serve` to start a standalone server. If you have the
corro TUI running, `corro serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Corro Code [IDE](/docs/ide) plugins.
...
The corro server exposes the following APIs.
