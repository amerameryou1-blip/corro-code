---
title: SDK
description: Client JS de type sécurisé pour le serveur corro.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK corro JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler corro par programme.
...
Créez une instance de corro :

```javascript
import { createOpencode } from "@corro-code/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `corro.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@corro-code/sdk"

const corro = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${corro.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de corro, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@corro-code/sdk"
...
```typescript
import type { Session, Message, Part } from "@corro-code/sdk"
```
