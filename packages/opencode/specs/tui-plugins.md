...
- Author package entrypoint is `@corro-ai/plugin/tui`.
...
- Import types from `@corro-ai/plugin/tui`.
- `@corro-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@corro-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.corro` dir or `OPENCODE_CONFIG_DIR`, Corro Code installs `@corro-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@corro-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@corro-ai/plugin/tui`.
