...
Mit Corro Code kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Corro Code das `corro`-Theme.
...
Corro Code liefert mehrere eingebaute Themes mit.
...
- Corro Code optisch ans Terminal anpassen wollen
...
Corro Code unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/corro/themes/*.json` oder `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.corro/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.corro/themes/*.json`
