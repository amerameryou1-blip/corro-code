---
title: CLI
description: Opções e comandos da CLI do corro.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do corro, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
corro
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o corro programaticamente.
...
Inicie a interface de usuário do terminal do corro.
...
A CLI do corro também possui os seguintes comandos.

---

### agent

Gerencie agentes para o corro.
...
Anexe um terminal a um servidor backend do corro já em execução, iniciado via comandos `serve` ou `web`.

```bash
corro attach [url]
```

Isso permite usar o TUI com um backend corro remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `corro`) |
...
O corro é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `corro auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

Quando o corro é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no corro.
...
Execute o corro em modo não interativo passando um prompt diretamente.
...
```bash "corro run"
corro run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `corro serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
corro run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor corro em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `corro`) |
...
Inicie um servidor corro sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
corro serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do corro sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `corro`).
...
Liste todas as sessões do corro.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do corro.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do corro.

```bash
corro import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do corro.
...
Inicie um servidor corro sem cabeça com uma interface web.

```bash
corro web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o corro através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `corro`).
...
Desinstale o corro e remova todos os arquivos relacionados.
...
Atualiza o corro para a versão mais recente ou uma versão específica.
...
A CLI do corro aceita as seguintes flags globais.
...
O corro pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `corro`) |
