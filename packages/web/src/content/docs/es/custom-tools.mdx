...
description: Cree herramientas que LLM pueda llamar en corro.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de corro como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.corro/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/corro/tools/`.
...
```ts title=".corro/tools/database.ts" {1}
import { tool } from "@corro-code/plugin"
...
```ts title=".corro/tools/project.ts" {8}
import { tool } from "@corro-code/plugin"
...
```python title=".corro/tools/add.py"
import sys
...
```ts title=".corro/tools/python-add.ts" {10}
import { tool } from "@corro-code/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".corro/tools/add.py")
