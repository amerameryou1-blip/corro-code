# Corro Code V2 Effect Plugin API

The Effect plugin API grants plugins two in-process capabilities:

- `hook` installs behavior at an Corro Code extension point.
...
```ts
import { define } from "@corro-code/plugin/v2/effect"
...
Corro Code rebuilds the domain when a transform is registered or disposed. A rebuild starts from fresh domain state and runs every active transform in registration order.
