---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w corro.
---

import config from "../../../../config.mjs"
export const console = config.console

corro używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji corro.
...
w `~/.local/share/corro/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim corro
config.
...
Corro Code Zen to lista modeli dostarczonych przez zespół corro, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z corro. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Corro Code Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Corro Code Zen` i przejdź do [corrocode.dev/auth](https://corrocode.dev/zen).
...
Działa jak każdy inny dostawca w corro i jest całkowicie opcjonalny w użyciu.

---

## Corro Code Go

Corro Code Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół corro, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z corro.

1. Uruchom polecenie `/connect` w TUI, wybierz `Corro Code Go` i przejdź do [corrocode.dev/auth](https://corrocode.dev/zen).
...
Aby używać Amazon Bedrock z corro:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania corro:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `corro.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w corro nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować corro do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod corro działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji corro.
...
corro używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować corro
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`corro.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [corro-gitlab-auth](https://www.npmjs.com/package/corro-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem corro:
...
Aby używać Google Vertex AI z corro:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania corro.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą corro, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [corro-helicone-session](https://github.com/H2Shami/corro-helicone-session), aby automatycznie rejestrować każdą konwersację corro jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja corro będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować corro tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować corro tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować corro tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem corro. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/corro).
:::
...
Aby korzystać z Ollama Cloud z corro:
...
3. Skopiuj klucz API do użycia w corro.
...
6. **Ważne**: Przed użyciem modeli chmurowych w corro musisz pobrać informacje o modelu lokalnie:
...
Corro Code Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół corro. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Corro Code Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Corro Code Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API corro.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji corro.
...
5. Możesz je także dostosować za pomocą konfiguracji corro. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z corro:
...
Możesz także dostosować modele za pomocą konfiguracji corro. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z corro. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `corro.json` w katalogu projektu:
...
Pola `limit` pozwalają corro zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `corro auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację corro i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji corro.
