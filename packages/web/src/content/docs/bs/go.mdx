...
Corro Code Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Corro Code-u. Pretplatite se na Corro Code Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Corro Code.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Corro Code-om.
:::
...
Corro Code Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

Corro Code Go radi kao bilo koji drugi provajder u Corro Code-u.

1. Prijavite se na **<a href={console}>Corro Code Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Corro Code Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Corro Code Go.
:::
...
Corro Code Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Corro Code konfiguraciji
koristi format `corro-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`corro-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Corro Code Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Corro Code koristite i bilo kojeg drugog provajdera.
