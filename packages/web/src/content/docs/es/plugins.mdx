---
title: Complementos
description: Escriba sus propios complementos para extender Corro Code.
---

Los complementos le permiten extender Corro Code al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Corro Code.
...
- `.corro/plugins/` - Complementos a nivel de proyecto
- `~/.config/corro/plugins/` - Complementos globales
...
```json title="corro.json"
{
  ...
  "plugin": ["corro-helicone-session", "corro-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/corro/node_modules/`.
...
1. Configuración global (`~/.config/corro/corro.json`)
2. Configuración del proyecto (`corro.json`)
3. Directorio global de complementos (`~/.config/corro/plugins/`)
4. Directorio de complementos del proyecto (`.corro/plugins/`)
...
Corro Code ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".corro/plugins/my-plugin.ts"
...
```js title=".corro/plugins/example.js"
...
- `client`: Un cliente SDK corro para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar corro.
...
```js title=".corro/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "corro"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Corro Code, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que corro lea archivos `.env`:

```javascript title=".corro/plugins/env-protection.js"
...
```javascript title=".corro/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a corro:

```ts title=".corro/plugins/custom-tools.ts"
import { type Plugin, tool } from "@corro-code/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que corro puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para corro junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://corrocode.dev/docs/sdk) para obtener más detalles.
...
```ts title=".corro/plugins/compaction.ts"
import type { Plugin } from "@corro-code/plugin"
...
```ts title=".corro/plugins/custom-compaction.ts"
import type { Plugin } from "@corro-code/plugin"
