---
title: CLI
description: Corro Code CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Corro Code CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
corro
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Corro Code interagieren.
...
Starten Sie die Terminalbenutzeroberfläche Corro Code.
...
Der Corro Code CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Corro Code verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden Corro Code-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
corro attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Corro Code. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `corro`) |
...
Corro Code wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `corro auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/corro/auth.json` gespeichert.

```bash
corro auth login
```

Wenn Corro Code startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Corro Code sehen möchten.
...
Führen Sie Corro Code im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `corro serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Corro Code-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `corro`)                  |
...
Starten Sie einen Headless-Corro Code-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
corro serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Corro Code-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `corro`).
...
Corro Code-Sitzungen verwalten.
...
Listen Sie alle Corro Code-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Corro Code-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Corro Code-Freigabe-URL.

```bash
corro import <file>
```

Sie können aus einer lokalen Datei oder einer Corro Code-Freigabe-URL importieren.
...
Starten Sie einen Headless-Corro Code-Server mit einer Weboberfläche.

```bash
corro web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Corro Code zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `corro`).
...
Deinstallieren Sie Corro Code und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Corro Code auf die neueste Version oder eine bestimmte Version.
...
Der Corro Code CLI akzeptiert die folgenden globalen Flags.
...
Corro Code kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `corro`) |
