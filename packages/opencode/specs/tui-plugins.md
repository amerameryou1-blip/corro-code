...
- Author package entrypoint is `@corro-code/plugin/tui`.
...
```json
{
  "$schema": "https://corrocode.dev/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/corro-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@corro-code/plugin/tui`.
- `@corro-code/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@corro-code/plugin/tui"
...
- The value is a semver range checked against the running Corro Code version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.corro` is absent, no check is performed (backward compatible).
...
- `corro plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `corro plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.corro` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.corro`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.corro`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.corro` dir or `OPENCODE_CONFIG_DIR`, Corro Code installs `@corro-code/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@corro-code/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Corro Code's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Corro Code registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@corro-code/plugin/tui`.
...
- `message` is required; `title` defaults to `"corro"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `corro.default`.
...
- Local plugins persist installed themes under the local `.corro/themes` area near the plugin config source.
...
- Local smoke plugin: `.corro/plugins/tui-smoke.tsx`
- Local vim plugin: `.corro/plugins/tui-vim.tsx`
- Local smoke config: `.corro/tui.json`
- Local smoke theme: `.corro/plugins/smoke-theme.json`
