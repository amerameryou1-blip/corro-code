---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Corro Code.
---

Dodaci vam omogućavaju da proširite Corro Code spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Corro Code.
...
- `.corro/plugins/` - Dodaci na nivou projekta
- `~/.config/corro/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/corro/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/corro/corro.json`)
2. Konfiguracija projekta (`corro.json`)
3. Globalni direktorij dodataka (`~/.config/corro/plugins/`)
4. Direktorij dodataka projekta (`.corro/plugins/`)
...
Corro Code pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Corro Code.
...
Spriječite corro da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u corro:
...
Pomoćnik `tool` kreira prilagođeni alat koji corro može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za corro zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://corrocode.dev/docs/sdk) za detalje.
