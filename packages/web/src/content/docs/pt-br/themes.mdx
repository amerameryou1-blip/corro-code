...
Com o corro, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o corro usa nosso próprio tema `corro`.
...
O corro vem com vários temas embutidos.
...
- Querem que o corro corresponda à aparência do seu terminal
...
O corro suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/corro/themes/*.json` ou `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.corro/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.corro/themes/*.json`
