...
أنشئ الملف `.corro/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Corro Code أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Corro Code:
...
- على مستوى النظام: `~/.config/corro/commands/`
- لكل مشروع: `.corro/commands/`
...
يتضمن corro عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
