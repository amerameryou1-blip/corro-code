...
Corro Code Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Corro Code. Ti abboni a Corro Code Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare Corro Code.
...
Corro Code Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

Corro Code Go funziona come qualsiasi altro provider in Corro Code.

1. Accedi a **<a href={console}>Corro Code Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Corro Code Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Corro Code Go.
:::
...
Corro Code Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Corro Code config
utilizza il formato `corro-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`corro-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato Corro Code Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Corro Code.
