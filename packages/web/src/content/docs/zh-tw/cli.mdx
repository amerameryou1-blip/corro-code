---
title: CLI
description: Corro Code CLI 選項和指令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Corro Code CLI 在不帶任何參數執行時，預設啟動 [TUI](/docs/tui)。

```bash
corro
```

但它也接受本頁面中記錄的指令，使您可以透過程式方式與 Corro Code 進行互動。
...
將終端機連接到已透過 `serve` 或 `web` 指令啟動的 Corro Code 後端伺服器。

```bash
corro attach [url]
```

這允許將 TUI 與遠端 Corro Code 後端搭配使用。例如：
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本驗證使用者名稱（預設使用 `OPENCODE_SERVER_USERNAME` 或 `corro`） |
...
Corro Code 基於 [Models.dev](https://models.dev) 的供應商列表運作，因此您可以使用 `corro auth login` 為任何想要使用的供應商設定 API 金鑰。金鑰儲存在 `~/.local/share/corro/auth.json` 中。

```bash
corro auth login
```

Corro Code 啟動時會從憑證檔案載入供應商資訊，同時也會載入環境變數或專案中 `.env` 檔案中定義的金鑰。
...
使用 `--refresh` 旗標可以更新快取的模型列表。當供應商新增了模型並且您希望在 Corro Code 中看到它們時，此功能非常有用。
...
您還可以連接到正在執行的 `corro serve` 實例，以避免每次執行時 MCP 伺服器的冷啟動時間：
...
| <nobr><code>{"--attach"}</code></nobr>   |      | 連接到正在執行的 corro 伺服器（例如 http://localhost:4096）          |
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本驗證密碼（預設使用 `OPENCODE_SERVER_PASSWORD`）                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本驗證使用者名稱（預設使用 `OPENCODE_SERVER_USERNAME` 或 `corro`） |
...
啟動無介面的 Corro Code 伺服器以提供 API 存取。查看[伺服器文件](/docs/server)了解完整的 HTTP 介面。

```bash
corro serve
```

此指令啟動一個 HTTP 伺服器，提供對 Corro Code 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `corro`）。
...
顯示 Corro Code 工作階段的 Token 用量和費用統計資訊。
...
從 JSON 檔案或 Corro Code 分享連結匯入工作階段資料。

```bash
corro import <file>
```

您可以從本地檔案或 Corro Code 分享連結匯入。
...
啟動帶有 Web 介面的無介面 Corro Code 伺服器。

```bash
corro web
```

此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 Corro Code。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `corro`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `corro`） |
