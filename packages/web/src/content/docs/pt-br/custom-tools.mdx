...
description: Crie ferramentas que o LLM pode chamar no corro.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do corro, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.corro/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/corro/tools/`.
