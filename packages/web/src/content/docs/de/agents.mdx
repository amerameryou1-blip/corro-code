...
Es gibt zwei Arten von Agenten in Corro Code; Primäragenten und Subagenten.
...
Corro Code verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `corro.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Corro Code modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Corro Code-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Corro Code-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Corro Code Zen](/docs/zen) verwenden, würden Sie `corro/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `corro models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/corro).
:::
