---
title: CLI
description: Corro Code CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Corro Code CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
corro
```

But it also accepts commands as documented on this page. This allows you to interact with Corro Code programmatically.
...
Start the Corro Code terminal user interface.
...
The Corro Code CLI also has the following commands.

---

### agent

Manage agents for Corro Code.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.corro/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running Corro Code backend server started via `serve` or `web` commands.

```bash
corro attach [url]
```

This allows using the TUI with a remote Corro Code backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `corro`) |
...
Corro Code is powered by the provider list at [Models.dev](https://models.dev), so you can use `corro auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/corro/auth.json`.

```bash
corro auth login
```

When Corro Code starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Corro Code.
...
Run corro in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `corro serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running corro server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `corro`) |
...
Start a headless Corro Code server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
corro serve
```

This starts an HTTP server that provides API access to corro functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `corro`).
...
Delete an Corro Code session.

```bash
corro session delete <sessionID>
```
...
Show token usage and cost statistics for your Corro Code sessions.
...
Import session data from a JSON file or Corro Code share URL.

```bash
corro import <file>
```

You can import from a local file or an Corro Code share URL.
...
Start a headless Corro Code server with a web interface.

```bash
corro web
```

This starts an HTTP server and opens a web browser to access Corro Code through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `corro`).
...
Fetch and checkout a GitHub PR branch, then run Corro Code.
...
Uninstall Corro Code and remove all related files.
...
Updates corro to the latest version or a specific version.
...
The corro CLI takes the following global flags.
...
Corro Code can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `corro`) |
