...
Agent Skills erlauben Corro Code, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Corro Code sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Corro Code vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.corro/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/corro/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.corro/skills/git-release/SKILL.md` zum Beispiel so:
...
Corro Code listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `corro.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `corro.json`):
