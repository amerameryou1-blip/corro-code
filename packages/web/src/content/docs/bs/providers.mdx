---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Corro Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Corro Code koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Corro Code konfiguraciji.
...
u `~/.local/share/corro/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Corro Code
config.
...
Corro Code Zen je lista modela koje je obezbedio Corro Code tim koji su bili
testirano i potvrđeno da dobro radi sa Corro Code. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Corro Code Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite corro i idite na [corrocode.dev/auth](https://corrocode.dev/auth).
...
Radi kao i svaki drugi provajder u Corro Code i potpuno je opcionalan za korištenje.

---

## Corro Code Go

Corro Code Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Corro Code tim i koji su testirani i verificirani da dobro rade s Corro Code-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Corro Code Go` i idite na [corrocode.dev/auth](https://corrocode.dev/zen).
...
Da biste koristili Amazon Bedrock s Corro Code:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete corro:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `corro.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Corro Code.
:::
...
Možete konfigurirati corro za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi corro ispravno radio.
   :::
...
Također možete dodati modele kroz svoju corro konfiguraciju.
...
da pohrani token u corro auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Corro Code koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Corro Code
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`corro.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [corro-gitlab-auth](https://www.npmjs.com/package/corro-gitlab-auth).

##### Konfiguracija

Prilagodite putem `corro.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s corro:
...
Za korištenje Google Vertex AI s Corro Code:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće corro.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [corro-helicone-session](https://github.com/H2Shami/corro-helicone-session) da automatski prijavite svaki Corro Code razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Corro Code razgovor naveden kao zasebna sesija.
...
Možete konfigurirati corro za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati corro za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati corro za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Corro Code. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/corro) za detalje.
:::
...
Da biste koristili Ollama Cloud s Corro Code:
...
3. Kopirajte API ključ za korištenje u Corro Code.
...
6. **Važno**: Prije upotrebe modela oblaka u Corro Code, morate lokalno povući informacije o modelu:
...
Corro Code Zen je lista testiranih i verifikovanih modela koju je obezbedio Corro Code tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Corro Code Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Corro Code Zen**.

```txt
   /connect
```

3. Unesite svoj Corro Code API ključ.
...
Također možete dodati dodatne modele putem vaše corro konfiguracije.
...
5. Također ih možete prilagoditi putem vaše corro konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju corro konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s corro-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `corro.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Corro Code da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `corro auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Corro Code konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj corro konfiguraciji.
