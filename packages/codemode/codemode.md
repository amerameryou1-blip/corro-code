...
This is the living design and status document for `@corro-code/codemode` and its existing V2 Corro Code adapter.
...
### V2 Corro Code adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/corro/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
