...
U Corro Code mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Corro Code koristi nasu `corro` temu.
...
Corro Code dolazi sa vise ugradenih tema.
...
- Zele da Corro Code odgovara izgledu njihovog terminala
...
Corro Code podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/corro/themes/*.json` ili `$XDG_CONFIG_HOME/corro/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.corro/themes/*.json`
4. **Trenutni radni direktorij** - `./.corro/themes/*.json`
