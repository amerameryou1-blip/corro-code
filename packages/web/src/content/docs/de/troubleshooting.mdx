...
Wenn Corro Code Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\corro\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `corro --log-level DEBUG`.

---

## Speicher

corro speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/corro/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\corro`
...
Corro Code Desktop startet im Hintergrund einen lokalen Corro Code-Server (`corro-cli`-Sidecar).
...
- Nur macOS: `Corro Code`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/corro/corro.jsonc` (oder `~/.config/corro/corro.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/corro/corro.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\corro\corro.jsonc`
...
Corro Code kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/corro/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\corro\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Corro Code ihn neu aufbauen kann.

1. Beende Corro Code Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/corro`
- **Linux**: loesche `~/.cache/corro` (oder fuehre aus: `rm -rf ~/.cache/corro`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\corro`

3. Starte Corro Code Desktop neu.

---

### Server-Verbindungsprobleme beheben

Corro Code Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `corro.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Corro Code Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Corro Code-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Corro Code Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Corro Code in den OS-Einstellungen aktiviert sind, und
...
1. Beende Corro Code Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Corro Code Desktop):

- `corro.settings.dat` (desktop default server URL)
- `corro.global.dat` und `corro.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Corro Code hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `corro models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\corro`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. corro installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\corro`

2. Starte corro neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt corro, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
corro erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
